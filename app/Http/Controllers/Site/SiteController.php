...
class SiteController extends Controller
{
        ...
        if(isset($tag)) {
            ...
            if($data->total() > 0) {
                ...
                if(empty($tag->meta_title)) {
                    if($page > 1) {
                        $tag->meta_title = $tagName.' trang '.$page.' | blogvansu.com';
                    } else {
                        $tag->meta_title = $tagName.' | blogvansu.com';
                    }
                }
    ...
    public function page(Request $request, $slug)
    {
        ...
        if(isset($type)) {
            ...
            if(empty($type->meta_title)) {
                if($page > 1) {
                    $type->meta_title = $typeName.' trang '.$page.' | blogvansu.com';
                } else {
                    $type->meta_title = $typeName.' | blogvansu.com';
                }
            }
        ...
        if($post) {
            ...
            if(empty($post->meta_title)) {
                $post->meta_title = $postName.' | blogvansu.com';
            }
    ...
    public function search(Request $request)
    {
        ...
        //auto meta tag for seo
        if($page > 1) {
            $search['meta_title'] = 'Tìm kiếm: '.$request->s.' trang '.$page.' | blogvansu.com';
        } else {
            $search['meta_title'] = 'Tìm kiếm: '.$request->s.' | blogvansu.com';
        }
